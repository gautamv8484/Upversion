...
export default function ListingDetailPage() {
  ...
  const {
    listings, currentUser, getCompanyById, toggleSaveListing,
    isListingSaved, applyToListing, hasApplied, incrementViewCount,
    getApplicationsForListing, getListingsForCompany
  } = useApp();
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-20 text-center">
        <div className="max-w-md mx-auto">
          ...
          <p className="text-[#5F6368] mt-2 mb-6">This listing may have been removed or the URL is incorrect.</p>
          <Link to="/explore" className="inline-flex items-center gap-2 bg-[#111111] text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-[#1A1A1A] transition-colors">
  ...
  const similarListings = listings
    .filter(l => l.id !== listing.id && l.status === 'active' && (l.category === listing.category || l.type === listing.type))
    .slice(0, 3);

  const companyListings = company ? getListingsForCompany(company.id).filter(l => l.status === 'active').length : 0;
  ...
  const handleResumeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowed.includes(file.type)) { setApplyError('Only PDF, DOC, DOCX allowed'); return; }
    if (file.size > 5 * 1024 * 1024) { setApplyError('Max 5MB'); return; }
    setResumeFile(file);
    setApplyError('');
  };
  ...
  const handleApply = () => {
    setApplyError('');
    if (!currentUser) { navigate('/login'); return; }
    if (currentUser.role !== 'candidate') { setApplyError('Only candidates can apply'); return; }
    if (isFreelance) {
    ...
    setTimeout(() => {
      ...
      setSubmitting(false);
      if (success) { setApplySuccess(true); }
      else { setApplyError('You have already applied'); }
    }, 800);
  };
  ...
  const closeModal = () => {
    ...
    setCoverLetter(''); setProposalText('');
    setExpectedBudget(''); setEstimatedTimeline('');
    setPortfolioUrl(''); setResumeFile(null); setApplyError('');
  };

  const daysRemaining = () => {
    const diff = Math.ceil((new Date(listing.deadline).getTime() - Date.now()) / 86400000);
    ...
    <>
        ...
        <div className="border-b border-[#E5E7EB] bg-[#FAFAFA]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3">
            <div className="flex items-center gap-2 text-xs text-[#5F6368]">
              <Link to="/explore" className="hover:text-[#111111] transition-colors">Explore</Link>
              <ChevronRight size={12} />
              <Link to={'/' + (listing.type === 'job' ? 'jobs' : listing.type === 'internship' ? 'internships' : 'freelance')} className="hover:text-[#111111] transition-colors capitalize">
                {listing.type === 'job' ? 'Jobs' : listing.type === 'internship' ? 'Internships' : 'Freelance'}
              </Link>
              <ChevronRight size={12} />
              <span className="text-[#111111] font-medium truncate max-w-[200px]">{listing.title}</span>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex-1 min-w-0 space-y-6">
              ...
              <div className="bg-white border border-[#E5E7EB] rounded-xl p-6 sm:p-8">
                    ...
                    <Link to={'/company/' + (company?.slug || '')} className="shrink-0">
                      <div className="w-14 h-14 bg-gray-100 rounded-xl border border-[#E5E7EB] flex items-center justify-center overflow-hidden hover:border-gray-300 transition-colors">
                        {(company?.logoFile || company?.logoUrl) ? (
                          ...
                          <span className="text-xl font-bold text-gray-700">{company?.companyName?.charAt(0) || '?'}</span>
                        )}
                      </div>
                    </Link>
                    <div>
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  ...
                  {listing.experienceLevel !== 'any' && (
                    <Badge variant="outline" size="md" className="capitalize">{listing.experienceLevel} Level</Badge>
                  )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-[#F3F4F6]">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-[10px] text-[#5F6368] uppercase tracking-wide">Location</p>
                      <p className="text-sm font-medium text-[#111111]">{listing.location}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 bg-[#F3F4F6] rounded-lg flex items-center justify-center">
                      <DollarSign size={14} className="text-[#5F6368]" />
                    </div>
                    <div>
                      <p className="text-[10px] text-[#5F6368] uppercase tracking-wide">
                        {listing.type === 'job' ? 'Salary' : listing.type === 'internship' ? 'Stipend' : 'Budget'}
                      </p>
                      <p className="text-sm font-medium text-[#111111]">{salary !== 'Not specified' ? salary : 'Undisclosed'}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 bg-[#F3F4F6] rounded-lg flex items-center justify-center">
                    ...
                    <div>
                      <p className="text-[10px] text-[#5F6368] uppercase tracking-wide">Deadline</p>
                      <p className={'text-sm font-medium ' + (isExpired ? 'text-red-600' : 'text-[#111111]')}>{deadlineStatus}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 bg-[#F3F4F6] rounded-lg flex items-center justify-center">
                    ...
                    <div>
                      <p className="text-[10px] text-[#5F6368] uppercase tracking-wide">Posted</p>
                      <p className="text-sm font-medium text-[#111111]">{timeAgo(listing.createdAt)}</p>
                    </div>
                  </div>
                </div>

                <div className="flex items-center gap-4 mt-4 text-xs text-[#5F6368]">
                  {listing.viewCount ? <span className="flex items-center gap-1"><Eye size={13} /> {listing.viewCount} views</span> : null}
                  <span className="flex items-center gap-1"><Users size={13} /> {applicationsCount} applicant{applicationsCount !== 1 ? 's' : ''}</span>
                </div>
              </div>

              {/* Description */}
              <div className="bg-white border border-[#E5E7EB] rounded-xl p-6 sm:p-8">
                <h2 className="text-lg font-semibold text-[#111111] font-[family-name:var(--font-heading)] mb-4">About This Role</h2>
                <p className="text-sm text-[#5F6368] leading-relaxed whitespace-pre-line">{listing.description}</p>
              </div>

              {/* Responsibilities */}
              {listing.responsibilities.length > 0 && (
                <div className="bg-white border border-[#E5E7EB] rounded-xl p-6 sm:p-8">
                  <h2 className="text-lg font-semibold text-[#111111] font-[family-name:var(--font-heading)] mb-4">Responsibilities</h2>
              ...
              {listing.requirements.length > 0 && (
                <div className="bg-white border border-[#E5E7EB] rounded-xl p-6 sm:p-8">
                  <h2 className="text-lg font-semibold text-[#111111] font-[family-name:var(--font-heading)] mb-4">Requirements</h2>
                  <ul className="space-y-3">
              ...
              {/* Skills */}
              <div className="bg-white border border-[#E5E7EB] rounded-xl p-6 sm:p-8">
                <h2 className="text-lg font-semibold text-[#111111] font-[family-name:var(--font-heading)] mb-4">Skills Required</h2>
              ...
              {listing.perks.length > 0 && (
                <div className="bg-white border border-[#E5E7EB] rounded-xl p-6 sm:p-8">
                  <h2 className="text-lg font-semibold text-[#111111] font-[family-name:var(--font-heading)] mb-4">Perks & Benefits</h2>
              ...
              {similarListings.length > 0 && (
                <div>
                  <h2 className="text-lg font-semibold text-[#111111] font-[family-name:var(--font-heading)] mb-4">Similar Opportunities</h2>
                ...
                <div className="bg-white border border-[#E5E7EB] rounded-xl p-6">
                  {salary !== 'Not specified' && (
                    <div className="mb-4">
                      ...
                      <p className="text-2xl font-bold text-[#111111] font-[family-name:var(--font-heading)]">{salary}</p>
                    </div>
                  )}
                  ...
                  <div className="mt-4 pt-4 border-t border-[#F3F4F6]">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className={'font-medium ' + (isExpired ? 'text-red-600' : 'text-[#111111]')}>{formatDate(listing.deadline)}</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="bg-white border border-[#E5E7EB] rounded-xl overflow-hidden">
                    <div className="h-20 relative overflow-hidden">
                      {(company.coverImageFile || company.coverImageUrl) ? (
                        <img src={company.coverImageFile || company.coverImageUrl} alt="Cover" className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full bg-gradient-to-br from-[#1A1A1A] to-[#333]" />
                      )}
                    </div>
                    <div className="p-5 -mt-6">
                      <div className="w-12 h-12 border-2 border-white rounded-lg bg-white shadow-sm overflow-hidden flex items-center justify-center mb-3">
                        {(company.logoFile || company.logoUrl) ? (
                          <img src={company.logoFile || company.logoUrl} alt={company.companyName} className="w-full h-full object-cover" />
                        ) : (
                      ...
                      <p className="text-xs text-[#5F6368] mt-2 leading-relaxed" style={{
                        display: '-webkit-box',
                        WebkitLineClamp: 3,
                      ...
                      <div className="space-y-2 mt-4 pt-4 border-t border-[#F3F4F6]">
                        <div className="flex items-center gap-2 text-xs text-[#5F6368]"><MapPin size={12} /> {company.location}</div>
                        <div className="flex items-center gap-2 text-xs text-[#5F6368]"><Users size={12} /> {company.size} employees</div>
                        <div className="flex items-center gap-2 text-xs text-[#5F6368]"><Briefcase size={12} /> {companyListings} active listing{companyListings !== 1 ? 's' : ''}</div>
                        {company.website && (
      ...
      {showApplyModal && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden animate-fade-in flex flex-col" style={{ maxHeight: '90vh' }}>
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#E5E7EB] shrink-0">
              <div>
                ...
                <p className="text-xs text-[#5F6368] mt-0.5">{listing.title} at {company?.companyName}</p>
              </div>
              ...
              {applySuccess ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-[#5F6368] mt-2 max-w-sm mx-auto">
                    Your {isFreelance ? 'proposal' : 'application'} for <strong>{listing.title}</strong> has been sent to {company?.companyName}.
                  </p>
                  ...
                  {isFreelance ? (
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="text-xs font-medium text-[#5F6368] mb-1.5 block">Budget ($) <span className="text-red-500">*</span></label>
                    ...
                    <div>
                      <label className="text-xs font-medium text-[#5F6368] mb-1.5 block">Cover Letter <span className="text-red-500">*</span></label>
                      <textarea
                        value={coverLetter}
